...
public class SelfAnalysis extends JFrame implements ActionListener{
    ...
    Color squareCol = getColorFromString(config.getProperty("Darksq"));
    boolean sounds = config.getProperty("Sound").equals("true") ? true : false;
    Stockfish stockfish = new Stockfish();
    JLabel valuationlabel, bestmove;
    JButton deletebtn;
    ...
    private void addboard(){
        ...
        layeredPane.setPreferredSize(new Dimension(650, 650));
    ...
    private void addsidepanel(){
        ...
        bestmove = new JLabel();
        bestmove.setPreferredSize(new Dimension(170,50));
        sidepanel.add(bestmove);

        valuationlabel = new JLabel();
        valuationlabel.setPreferredSize(new Dimension(170,50));
        stockfish.SetEvaluation("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1", valuationlabel,bestmove);
        sidepanel.add(valuationlabel);
    }
    ...
    private void sidebuttons(){
            ...
            public void actionPerformed(ActionEvent e) {
                if(!redo.isEmpty()){
                    ...
                    if(redo.isEmpty()){
                        ...
                        enablebuttons(temp);
                        updatedisplay(board);
                        Position p = savedpositions.get(savedpositions.size()-1);
                        stockfish.SetEvaluation(p.fen,valuationlabel,bestmove);
                    } else {
                        disableallbuttons();
                        updatedisplay(redo.peek().board);
                        Position p = redo.peek();
                        stockfish.SetEvaluation(p.fen,valuationlabel,bestmove);
                    }

                    updatecurrentdot();
                } else {
                    redobtn.setText("<html>Nothing <br>to redo!<html>");
                    redobtn.setFont(new Font("Arial", Font.BOLD, 11));
        ...
        undobtn.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if(!undo.isEmpty()){
                    ...
                    undobtn.setText("<html>Nothing <br>to undo!<html>");
                    undobtn.setFont(new Font("Arial", Font.BOLD, 11));
                    java.util.Timer timer = new java.util.Timer();
                    timer.schedule(new TimerTask() {
        ...
        deletebtn = new JButton("Delete Last Move");
        ...
        deletebtn.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if(!undo.isEmpty() && redo.isEmpty()){
                    ...
                    replaceboard(savedpositions.get(savedpositions.size()-1).board);
                    undo.pop();
                    updatecurrentdot();
                    removetablecell();
                    updatedisplay(board);
                    ...
                    if(undo.isEmpty())deletebtn.setText("Nothing to delete!");
                    else deletebtn.setText("Go to last move!");
                    java.util.Timer timer = new java.util.Timer();
                    timer.schedule(new TimerTask() {

                        @Override
                        public void run() {
                            deletebtn.setText("Delete Last Move");
                        }
        ...
        addkeylisteners(undobtn,redobtn,deletebtn);
    ...
    private void addkeylisteners(JButton undobtn, JButton redobtn, JButton deletebtn){
        getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke("RIGHT"), "rightArrowPressed");

        getRootPane().getActionMap().put("rightArrowPressed", new AbstractAction() {
            @Override
            public void actionPerformed(ActionEvent e) {
                redobtn.doClick();
            }
        ...
        getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke("LEFT"), "leftArrowPressed");

        getRootPane().getActionMap().put("leftArrowPressed", new AbstractAction() {
            @Override
            public void actionPerformed(ActionEvent e) {
                undobtn.doClick();
            }
        ...
        getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke("BACK_SPACE"), "backSpacePressed");

        getRootPane().getActionMap().put("backSpacePressed", new AbstractAction() {
            @Override
            public void actionPerformed(ActionEvent e) {
                deletebtn.doClick();
            }
    ...
    private void replaceboard(Piece[][] newboard){
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
    ...
    private void removetablecell(){
        ...
        if(!tablemodel.getValueAt(lastrow, 1).equals("")){
    ...
    public void makeMove(JButton sourcebutton, int x, int y){
        //if the selected button is a highlighted button where the user can move
        if(possiblemoves.contains(sourcebutton)){

            if(selected.ispawn){//relevant to know the board of the past moves of the pawn in order to judge for en Passant
            if (savedpositions.size()>1) {
                var = savedpositions.get(savedpositions.size()-2).board;
            }
            //Creating a new Pawn object for the selected piece
                ...
                for (int[] pos : selectedpiece.checkallmoves(board,var)) {
                    if(boardbuttons[pos[0]][pos[1]]==sourcebutton){
                        ...
                        if (board[pos[0]][pos[1]].isempty) {
                            displayMoves(pos[0],pos[1],false,false,-1,"");
                        }else{
                            displayMoves(pos[0],pos[1],true,false,-1,"");   
                        }
                        //making the move
            ...
            //Creating a new Rook object for the selected piece if the selected piece is a Rook
                ...
                if(board[x][y].isRook){
                    if ((x==7 && y==7) || (x==0 && y==7) ){ 
                        ...
                        castleshort();
                        displayMoves(-1,-1,false,false,0,"");
                    } else if((x==0 && y==0) || (x==7 && y==0)){
                        //calling the method for castling long
                        castlelong();
                        displayMoves(-1,-1,false,false,1,"");
                    }
                }
            }
            isSelected = false;
            ...
            if(KingInCheck(board)){
                if(CheckMate()){
                    endgame(whitetomove ? false : true,false);
                }
            }
             */
            ...
            /*
            if(findAllPossibleMoves().size()==0 && !KingInCheck(board)){
                //stalemate
            }
             */
             ...
             */
            //test
            //System.out.println(new FenString().createFEN(board, whitetomove, castlingrights, fiftymoverule, fenmovecounter, passantstring()));
            stockfish.SetEvaluation(savedpositions.get(savedpositions.size()-1).fen, valuationlabel, bestmove);
        }
    }
    ...
    public void displayMoves(int x , int y, boolean capture, boolean promotion, int castle, String promotiontype){
        ...
        if (whitetomove) {
            movetracker++;
            tablemodel.addRow(new Object[]{movetracker + ". " + movestr.toString(),""});
        } else {
            fenmovecounter++;
    ...
    private void updatecurrentdot(){
        ...
        int currentrow = (undo.size()-1)/2;
        int currentcol = (undo.size()-1)%2;
    ...
    public void promotion(Piece piece, int x, int y){
            ...
            b.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    if (board[x][y].isempty) {
                        displayMoves(x,y,false,true,-1,b.getName());
                    }else{
                        displayMoves(x,y,true,true,-1,b.getName());   
                    }
    ...
    if(whitetomove && !KingInCheck(board) && board[7][7].isRook){
                ...
                for (int j = 0; j < 8; j++) {
                    if(!currentboard[i][j].isempty && currentboard[i][j].white){
                        if(currentboard[i][j].isQueen){
                            ...
                            Pawn selected = (Pawn) currentboard[i][j];
                            for (int[] pos : selected.checkallmoves(currentboard,savedpositions.size()>1 ? savedpositions.get(savedpositions.size()-2).board : null)) {
                                if(pos[0]==kingpos[0] && pos[1]==kingpos[1]){
                                    return true;
                                }
                            }
