...
public class Bots extends JFrame implements ActionListener{
    ...
    Bots(String difficulty,boolean startcolor){
        ...
        Position p = new Position(board, fenmovecounter, fiftymoverule, castlingrights, timer1.isRunning(), savedpositions.size()>0 ? savedpositions.get(savedpositions.size()-1).board : null);
        savedpositions.add(p);

        if(botcolor)simulatemove(savedpositions.get(savedpositions.size()-1).fen);
    }
        ...
        timer1 = new Timer(1000, new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if ((time1[0]==10 && time1[1]==0) || (time2[0]==10 && time2[1]==0)) {
                    if(sounds)play("chess\\soundfx\\tenseconds.wav");
                }
            }
        });
        //Starting the timer
    ...
    private void addsidepanel(){
        ...
        buttonspanel.setPreferredSize(new Dimension(195,100));
    ...
    public void endgame(boolean white, boolean draw){
        if(sounds)play("chess\\soundfx\\game-end.wav");
        if(draw){
    ...
    //Method for making the actions when a button is clicked 
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            //Allows the user to make a move by clicking the preferred button on the board
                ...
                findenabledbuttons(enabledbuttons);
                enablebuttons(enabledbuttons);
                if(botcolor==timer1.isRunning())simulatemove(savedpositions.get(savedpositions.size()-1).fen);
            }
        }
                    ...
                    if(selectedpiece.checkallmoves(board,var).size()>0){
                        ...
                        for (int[] pos : selectedpiece.checkallmoves(board,var)) {
                            if(boardbuttons[pos[0]][pos[1]]==sourcebutton){
                                ...
                                undo.add(new Position(board, fenmovecounter, fiftymoverule, castlingrights, timer1.isRunning(), savedpositions.size()>0 ? savedpositions.get(savedpositions.size()-1).board : null));
        ...
        private void simulatemove(String fen){
            String[] tomove = new String[1];
            new Thread(() -> {
                tomove[0]=stockfish.getBotMove(fen, difficulty);  // The method that runs in the background
                SwingUtilities.invokeLater(() -> {
                    // Code to run after we find the move
                    System.out.println(tomove[0]);
                    char[] str = tomove[0].toCharArray();
                    ...
                    if(board[reversemapX.get(str[1])][reversemapY.get(str[0])].isKing){
                        if(timer1.isRunning() && tomove[0].equals("e1g1"))boardbuttons[7][7].doClick();
                        else if(!timer1.isRunning() && tomove[0].equals("e8g8"))boardbuttons[0][7].doClick();
                        else if(timer1.isRunning() && tomove[0].equals("e1c1"))boardbuttons[7][0].doClick();
                        else if(!timer1.isRunning() && tomove[0].equals("e8c8"))boardbuttons[0][0].doClick();
        ...
        private boolean checkCastleLong(){
            ...
            if(timer1.isRunning() && !KingInCheck(board)){
                if (!whitekingMoved && !whiterook1 && board[7][0].isRook) {
                    Piece[][] tempboard = new Piece[8][8];
                    for (int i = 0; i < 8; i++) {
                        for (int j = 0; j < 8; j++) {
                            tempboard[i][j] = board[i][j];
            ...
            //if the king has not moved and the rook is in the starting position then castling long is possible
            } else if(timer2.isRunning() && !KingInCheck(board) && board[0][0].isRook){
        ...
        if(timer1.isRunning() && !KingInCheck(board) && board[7][7].isRook){
            ...
            //if the king has not moved and the rook is in the starting position then castling short is possible
        } else if(timer2.isRunning() && !KingInCheck(board) && board[0][7].isRook){
